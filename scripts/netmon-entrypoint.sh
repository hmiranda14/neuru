...
if [ -d "$APP/scripts" ]; then
    printf "# NetMon Python DB config — generated by the entrypoint (matches connection.php)\nDB = dict(\n    host     = '%s',\n    user     = '%s',\n    password = '%s',\n    database = '%s',\n)\n" \
        "$NM_DB_HOST" "$NM_DB_USER" "$NM_DB_PASS" "$NM_DB_NAME" > "$_pyc" \
        && echo "[entrypoint] generated scripts/nm_db_config.py (host=$NM_DB_HOST db=$NM_DB_NAME)"
