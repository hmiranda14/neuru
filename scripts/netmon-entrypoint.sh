...
# 0a0b) Python poller DB config. The pollers `from nm_db_config import DB` — a committed file that ships
#       with host='db' (the compose service name). An ALL-IN-ONE box (NEURU-in-a-Box: embedded MariaDB on
#       127.0.0.1) or any custom host would leave every poller unable to reach the DB
#       ("Can't connect to MySQL server on 'db:3306'"). Regenerate it from env EVERY boot so it always
#       matches connection.php. (connection.php is write-if-absent to preserve custom edits; this file is
#       purely generated, so we overwrite it.)
_pyc="$APP/scripts/nm_db_config.py"
if [ -d "$APP/scripts" ]; then
    # time_zone='+00:00' pins the poller session to UTC (PHP already runs SET time_zone='+00:00').
    # Critical on a box whose system TZ isn't UTC (e.g. America/Puerto_Rico): without it the pollers
    # write local-time recorded_at while PHP reads UTC → every poll looks hours stale → nodes render
    # DOWN and traffic/realtime views see no recent rows. NEURU storage is canonical UTC.
    printf "# NetMon Python DB config — generated by the entrypoint (matches connection.php; UTC session)\nDB = dict(\n    host      = '%s',\n    user      = '%s',\n    password  = '%s',\n    database  = '%s',\n    time_zone = '+00:00',\n)\n" \
        "$NM_DB_HOST" "$NM_DB_USER" "$NM_DB_PASS" "$NM_DB_NAME" > "$_pyc" \
        && echo "[entrypoint] generated scripts/nm_db_config.py (host=$NM_DB_HOST db=$NM_DB_NAME tz=UTC)"
    chown "www-data:${NEURU_USER}" "$_pyc" 2>/dev/null; chmod 644 "$_pyc" 2>/dev/null
fi
