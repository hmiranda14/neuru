<?php
// ─────────────────────────────────────────────────────────────────────────────
// NEURU — Windows Command Center (immersive, FLOATING-WIDGET dashboard). One box,
...
// layout and a dynamic detail dock when you click an item. Pulls windows.php JSON
...
if (!checkAccess($conn, 'windows')) { header('Location: /denied_access.php?page=windows'); exit; }
nm_win_ensure($conn);

$hid  = (int)($_GET['host'] ?? 0);
$host = $hid ? nm_win_host($conn, $hid) : null;
if (!$host) { header('Location: /windows.php'); exit; }
...
<div class="ws" id="ws">
  <div class="ws-bar">
    ...
    <a class="ws-btn" href="windows.php"><i class="fas fa-arrow-left"></i> Back</a>
  </div>
...
const POST = b => fetch('windows.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:b}).then(r=>r.json()).catch(()=>null);
...
function bVitals(){ const d=DATA.d; if(!d)return '<div class="muted">…</div>';
...
function bCpu(){ const d=DATA.d; if(!d)return '<div class="muted">…</div>'; const cc=d.top_cpu||[],mx=cc.reduce((a,x)=>Math.max(a,+x.pct||0),1);
...
function bNet(){ const d=DATA.d; if(!d)return '<div class="muted">…</div>'; const nc=d.net_conn||[];
  return `<div class="kv"><span>Throughput</span><b>↓ ${fmtKBs(d.net_rx)} &nbsp; ↑ ${fmtKBs(d.net_tx)}</b></div>`+
...
function bSensors(){ const d=DATA.d; if(!d)return '<div class="muted">…</div>'; const U={Voltage:' V',Clock:' MHz',Load:'%',Control:'%',Power:' W',Level:'%',Data:' GB',SmallData:' MB',Throughput:' MB/s',Current:' A',Energy:' mWh'};const TO=['Load','Clock','Power','Voltage','Control','Throughput','Data','Level'];
  ...
  if(!ks.length)return '<div class="muted">No extra sensors.</div>';
...
const WIDGETS={
  ...
  sensors:{t:'Sensors',i:'fa-gauge-high',w:340,h:440,render:bSensors,cnt:()=>DATA.d?(DATA.d.sensors||[]).length:''},
...
function mountWidgets(){ ORDER.forEach(id=>{
    if(id==='gpu' && !(DATA.gpu&&DATA.gpu.has_gpu)){ const e=widgetEl(id); if(e)e.remove(); return; }  // no GPU on this node → skip widget
...
async function loadLayout(){ try{ const r=await fetch('windows.php?api=layout_get').then(r=>r.json()); const def=defaultLayout();
...
async function loadAll(){
  ...
  const [dg,hl,ev,ifd,gp]=await Promise.all([
    fetch('windows.php?api=diag&host='+HID+'&_='+Date.now()).then(r=>r.json()).catch(()=>null),
    fetch('windows.php?api=health&host='+HID).then(r=>r.json()).catch(()=>null),
    fetch('windows.php?api=events&host='+HID+'&limit=40').then(r=>r.json()).catch(()=>null),
    fetch('windows.php?api=ifaces&host='+HID+'&_='+Date.now()).then(r=>r.json()).catch(()=>null),
  ...
  DATA={d:(dg&&dg.ok)?dg.data:DATA.d, h:hl, ev:ev, ifd:ifd, gpu:gp};
